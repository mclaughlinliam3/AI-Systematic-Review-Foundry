...
class MainWindow(QMainWindow):
    ...
    def _import_csv(self, data_type: str):
        """Import sources, topics, or statistics from CSV or XLSX."""
        path, _ = QFileDialog.getOpenFileName(
        ...
        try:
            if data_type == "sources":
                imported = import_sources_csv(path)
                count = len(imported)
                msg = QMessageBox(self)
                msg.setWindowTitle("Import Sources")
                msg.setText(f"Found {count} sources in file.\n\n"
                            f"Replace existing sources, or append to them?")
                replace_btn = msg.addButton("Replace", QMessageBox.ButtonRole.YesRole)
                append_btn = msg.addButton("Append", QMessageBox.ButtonRole.NoRole)
