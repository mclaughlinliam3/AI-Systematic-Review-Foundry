...
class MainWindow(QMainWindow):
    ...
    def _build_tabs(self):
        ...
        self.main_tab.sources_renumbered.connect(self._on_sources_renumbered)
        try:
            self.sources_tab.sources_changed.connect(self._mark_dirty)
        except AttributeError:
            pass

    def _on_sources_renumbered(self):
        """Source numbers/order changed in the Main tab — resync the
        Sources tab, whose table is rendered straight from
        session.sources in list order."""
        try:
    ...
    def _check_citation_validations(self) -> bool:
        ...
        """
        try:
            from citation_manager import get_cited_numbers_in_order
        except ImportError:
            return True  # can't check — allow export

        # Gather every citation number that appears in the review text.
        # This previously used extract_bracketed_numbers on the raw text,
        # which only matches single '[5]' citations — every number inside
        # a multi-citation like '[3,7]' skipped the approval gate.
        all_numbers = sorted(set(get_cited_numbers_in_order(self.session)))
        if not all_numbers:
...
def main():
    ...
    show_model_notice(window.config, window)
    sys.exit(app.exec())
