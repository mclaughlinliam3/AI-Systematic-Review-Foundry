...
def _build_citation(rec: Dict) -> str:
    ...
    if isinstance(authors, list) and authors:
        if len(authors) <= 3:
            parts.append(', '.join(a.strip() for a in authors if a))
        else:
    ...
    title = rec.get('title', '')
    if title:
        parts.append(_end_period(title))

    # Journal, Volume(Issue), Pages
    journal_parts = []
    journal = rec.get('journal', '')
    if journal:
        journal_parts.append(journal.strip())
    vol = rec.get('volume', '')
    issue = rec.get('issue', '')
    if vol and issue:
        journal_parts.append(f"{vol}({issue})")
    elif vol:
        journal_parts.append(str(vol).strip())
    sp = rec.get('start_page', '')
    ep = rec.get('end_page', '')
    if sp and ep:
        journal_parts.append(f"{sp}-{ep}")
    elif sp:
        journal_parts.append(str(sp).strip())
    if journal_parts:
        parts.append(_end_period(', '.join(journal_parts)))
