...
from PyQt6.QtGui import QFont, QCursor, QColor, QTextCursor, QTextDocument

# Model discovery lives in config_manager; the UI only presents it.
from config_manager import (newest_per_family, is_known_model,
                            profile_for_model, MODEL_TABLE_DATE)


# ═══════════════════════════════════════════════════════════════════
#  Startup model notice
# ═══════════════════════════════════════════════════════════════════

def show_model_notice(config, parent=None):
    """
    One-time-per-launch notice telling the user which model is active and
    how much this build actually knows about it.

    Call from the main window once the UI is up:

        from dialogs import show_model_notice
        show_model_notice(self.config, self)

    Silently does nothing if the user ticked "Don't show this again".
    """
    if not config.show_model_notice:
        return
    ...
    if known:
        recognised = (
            f"This model is in the built-in table, which was last checked "
            f"against Anthropic's documentation on <b>{MODEL_TABLE_DATE}</b>.")
    else:
        recognised = (
            f"This model is <b>not</b> in the built-in table (last checked "
            f"{MODEL_TABLE_DATE}), so only generic parameters will be sent. "
            f"That is the safe default, but it may not be optimal.")

    if profile['sampling']:
        sampling = ("Requests to it <b>will</b> include your Temperature "
                    "(or Top-P) setting.")
    else:
        sampling = ("Requests to it will <b>not</b> include Temperature or "
                    "Top-P. Anthropic removed those controls on newer "
                    "models, which reject them outright.")

    box = QMessageBox(parent)
    box.setWindowTitle("Claude Model In Use")
    box.setIcon(QMessageBox.Icon.Information)
    box.setTextFormat(Qt.TextFormat.RichText)
    box.setText(f"<b>Active model: {model}</b>")
    box.setInformativeText(
        f"{recognised}<br><br>"
        f"{sampling}<br><br>"
        f"Anthropic retires models and changes which parameters they "
        f"accept over time, so depending on today's date this model may no "
        f"longer be available, and its parameter support may have changed. "
        f"<b>Please verify against Anthropic's current documentation "
        f"rather than relying on this build.</b><br><br>"
        f"To switch models, or to ask the API which models your key can "
        f"use right now, open <b>Prompt Settings → API Settings</b> and "
        f"press <b>Check for New Models</b>.")
    ...
    if dont_show.isChecked():
        config.show_model_notice = False
...
class _FilterableList(QWidget):
    ...
    def add_item(self, text: str, data, selected: bool = False,
        ...
        # MUST add to list before setting selection — Qt ignores
        # setSelected() on items that don't belong to a widget yet.
...
class ContextConfigDialog(QDialog):
    ...
    def __init__(self, section_key: str, session,
        ...
        # Sections that rely on review_string in their prompts and should
        # NOT include source summaries by default
        self._no_default_summaries = section_key in (
    ...
    def _build_ui(self):
        ...
        # Determine whether the user has previously saved a context config.
        # This is the key distinction: if a config was saved, we honour it
        # exactly (even if the user selected zero summaries). If no config
        # was saved we apply defaults.
        _has_saved_config = not self._is_first_open
        ...
        for s in self.session.sources:
            ...
            # Selection logic:
            # 1) User previously saved a config → restore exactly
            # 2) First open + discussion/conclusion/abstract → empty
            # 3) First open + ratings exist → defer to auto-select below
            # 4) First open + no ratings → include non-rejected only
            #    (rating=True or rating=None/unscreened; never rating=False)
            is_excluded = src.get('rating') is False
            if _has_saved_config:
                sel = num in selected_summaries
            elif self._no_default_summaries:
                sel = False
            elif has_ratings:
                sel = False  # will be set by auto-select below
            else:
                sel = not is_excluded  # first open, no ratings → include non-rejected
        ...
        # and not a no-summary section (discussion/conclusion/abstract)
        if self._is_first_open and has_ratings \
           and not self._no_default_summaries:
...
# ═══════════════════════════════════════════════════════════════════
#  Link to Sections Dialog
# ═══════════════════════════════════════════════════════════════════

class LinkToSectionsDialog(QDialog):
    """
    Dialog for linking a topic or statistic to one or more review sections.
    Shows all available sections as checkboxes so the user can click rather
    than type.  On accept, returns the list of selected section keys and
    updates ``session.section_contexts`` so each chosen section's context
    includes the linked topic/stat — exactly as if the user had configured
    it through ContextConfigDialog.
    """

    def __init__(self, item_label: str, session,
                 current_linked: list = None, parent=None, *,
                 topic_id: str = None, stat_id: str = None):
        """
        Parameters
        ----------
        item_label : str
            Human-readable name shown in the title (e.g. "T3: Mortality rates").
        session : Session
            The live session object.
        current_linked : list[str]
            Section keys already linked (used to pre-check boxes).
        topic_id / stat_id : str
            Exactly one should be set — determines whether the link adds a
            topic or a statistic to each section's context.
        """
        super().__init__(parent)
        self.setWindowTitle(f"Link to Sections — {item_label}")
        self.setMinimumWidth(420)
        self.session = session
        self.topic_id = topic_id
        self.stat_id = stat_id
        self._current_linked = set(current_linked or [])
        self._checkboxes: list[tuple[QCheckBox, str]] = []  # (widget, key)
        self._build_ui()
    ...
    def _build_ui(self):
        ...
        hint = QLabel(
            "Select the review sections this item should be included in.\n"
            "Linked items are automatically added to each section's "
            "AI context configuration.")
        hint.setWordWrap(True)
        hint.setStyleSheet(
            "background: #eaf0fb; padding: 8px; border-radius: 4px;")
        ...
        for key, display in [("intro", "Introduction"),
                              ("discussion", "Discussion"),
                              ("conclusion", "Conclusion")]:
            cb = QCheckBox(display)
            cb.setChecked(key in self._current_linked)
            g_layout.addWidget(cb)
            self._checkboxes.append((cb, key))

        # ── Results subsections (dynamic) ──
        if self.session.results:
            g_layout.addWidget(QLabel(""))  # spacer
            g_layout.addWidget(QLabel("Results subsections:"))
            for i, rs in enumerate(self.session.results):
                sec = rs.get('section', f'Subsection {i + 1}')
                key = f"results_{i}"
                # Also recognise the legacy format "results: <name>"
                legacy_key = f"results: {sec}"
                pre_checked = (key in self._current_linked
                               or legacy_key in self._current_linked)
                cb = QCheckBox(f"Results: {sec}")
    ...
    def get_selected_keys(self) -> list[str]:
        """Return the canonical section keys that were checked."""
        return [key for cb, key in self._checkboxes if cb.isChecked()]

    def apply_to_session(self):
        """
        Write the link into ``session.section_contexts`` so that each
        selected section's context includes this topic/stat.  Sections
        that were *un*-checked are cleaned up as well.
        """
        selected = set(self.get_selected_keys())

        # Every section key that *could* be linked
        all_keys = {key for _, key in self._checkboxes}

        for key in all_keys:
            ctx = self.session.section_contexts.setdefault(key, {})
            ...
            if self.topic_id:
                ...
                if key in selected:
                    ...
                    if self.topic_id in topics:
                        topics.remove(self.topic_id)

            if self.stat_id:
                stats = ctx.setdefault('statistics', [])
                if key in selected:
                    if self.stat_id not in stats:
                        stats.append(self.stat_id)
                else:
                    if self.stat_id in stats:
                        stats.remove(self.stat_id)
...
class PromptSettingsDialog(QDialog):
    ...
    def _build_ui(self):
        ...
        # Hardcoded table, or the fetched list if Check for New Models
        # has been used. No network access happens on open.
        self._claude_models_all = self.config.claude_models
        claude_model_row = QHBoxLayout()
        claude_model_row.addWidget(self.claude_model_combo, 1)
        self.claude_refresh_btn = QPushButton("Check for New Models")
        self.claude_refresh_btn.clicked.connect(
            lambda: self._refresh_claude_models())
        claude_model_row.addWidget(self.claude_refresh_btn)
        claude_layout.addRow("Model:", claude_model_row)

        self.claude_show_old = QCheckBox("Show older / dated versions")
        self.claude_show_old.toggled.connect(
            lambda: self._populate_claude_combo())
        claude_layout.addRow("", self.claude_show_old)

        self.claude_auto_newest = QCheckBox(
            "Always use the newest available model")
        self.claude_auto_newest.setChecked(not self.config.claude_model_pinned)
        self.claude_auto_newest.toggled.connect(self._on_auto_newest_toggled)
        claude_layout.addRow("", self.claude_auto_newest)

        # activated fires only on user interaction, not on programmatic
        # repopulation, so it is the right signal for "the user picked one".
        self.claude_model_combo.activated.connect(
            lambda: self.claude_auto_newest.setChecked(False))

        self.claude_model_status = QLabel("")
        self.claude_model_status.setStyleSheet("color: #888;")
        self.claude_model_status.setWordWrap(True)
        claude_layout.addRow("", self.claude_model_status)

        self._populate_claude_combo()
        self.claude_model_status.setText(
            "Built-in model list. Press Check for New Models to ask the "
            "API what else is available.")
        ...
        self.startup_notice_check = QCheckBox(
            "Show the model notice at startup")
        self.startup_notice_check.setChecked(self.config.show_model_notice)
        api_layout.addWidget(self.startup_notice_check)
        ...
        self.m_max_tokens.setValue(
            master_params.get('max_tokens', 4096))
    ...
    def _on_auto_newest_toggled(self, on: bool):
        """Re-select the newest model when auto mode is switched back on."""
        if on and self.claude_model_combo.count():
            self.claude_model_combo.setCurrentIndex(0)

    def _populate_claude_combo(self):
        """
        Rebuild the model dropdown from whatever the last fetch returned.

        When auto mode is on the newest entry wins, so a stale saved model
        is not carried forward. When the user has pinned a choice, that
        choice is always kept as an entry -- even if a fetch failed -- so
        it can never be silently blanked.
        """
        pinned = not self.claude_auto_newest.isChecked()
        current = ((self.claude_model_combo.currentText().strip()
                    or self.config.claude_model) if pinned else "")
        models = (self._claude_models_all if self.claude_show_old.isChecked()
                  else newest_per_family(self._claude_models_all))

        self.claude_model_combo.blockSignals(True)
        self.claude_model_combo.clear()
        for m in models:
            self.claude_model_combo.addItem(m['id'])
            self.claude_model_combo.setItemData(
                self.claude_model_combo.count() - 1,
                m['display_name'], Qt.ItemDataRole.ToolTipRole)

        if current:
            idx = self.claude_model_combo.findText(current)
            if idx < 0:
                self.claude_model_combo.insertItem(0, current)
                idx = 0
            self.claude_model_combo.setCurrentIndex(idx)
        elif self.claude_model_combo.count():
            self.claude_model_combo.setCurrentIndex(0)
        self.claude_model_combo.blockSignals(False)

    def _refresh_claude_models(self):
        """Pull the live model list using the key currently in the field."""
        self.claude_model_status.setText("Fetching models…")
        self.claude_refresh_btn.setEnabled(False)
        QApplication.setOverrideCursor(Qt.CursorShape.WaitCursor)
        QApplication.processEvents()
        try:
            models, error = self.config.fetch_claude_models(
                api_key=self.claude_key_edit.text())
        finally:
            QApplication.restoreOverrideCursor()
            self.claude_refresh_btn.setEnabled(True)

        if error:
            # Keep whatever the cache holds rather than emptying the list.
            self.claude_model_status.setText(
                f"⚠ {error}"
                + (" — showing last known models"
                   if self._claude_models_all else ""))
            self._populate_claude_combo()
            return

        self._claude_models_all = models
        self._populate_claude_combo()
        unknown = [m['id'] for m in models if not is_known_model(m['id'])]
        note = ""
        if unknown:
            note = (f" · {len(unknown)} newer than this build "
                    f"(generic parameters will be used)")
        self.claude_model_status.setText(
            f"{len(models)} models available · showing "
            f"{self.claude_model_combo.count()}{note}")
    ...
    def _on_prompt_selected(self, row):
        ...
        self.p_max_tokens.setValue(params.get('max_tokens', 4096))
    ...
    def _apply_api_settings(self):
        ...
        self.config.set_claude_model(
            self.claude_model_combo.currentText(),
            pinned=not self.claude_auto_newest.isChecked())
        self.config.show_model_notice = \
            self.startup_notice_check.isChecked()
...
# ═══════════════════════════════════════════════════════════════════
#  Find Dialog  (Ctrl+F search for any QTextEdit)
# ═══════════════════════════════════════════════════════════════════

class FindDialog(QDialog):
    """
    Modeless find dialog that searches within the currently focused
    QTextEdit. Stays open so the user can keep pressing Next/Previous
    ...
    def _build_ui(self):
        ...
        prev_btn.setFixedWidth(65)
        prev_btn.clicked.connect(self._find_prev)
        prev_btn.setShortcut("Shift+Return")
        layout.addWidget(prev_btn)

        next_btn = QPushButton("▼ Next")
        next_btn.setFixedWidth(65)
        next_btn.clicked.connect(self._find_next)
        layout.addWidget(next_btn)

        close_btn = QPushButton("✕")
        close_btn.setFixedWidth(28)
        close_btn.clicked.connect(self._close_and_clear)
        layout.addWidget(close_btn)

    def activate_for(self, text_edit):
        """Show the dialog and bind it to the given QTextEdit."""
        self._target = text_edit
        # If there's selected text in the editor, pre-fill the search
    ...
    def _on_text_changed(self, text):
        ...
        # Move cursor to start and find first match
    ...
    def _highlight_all(self, text):
        """Count matches and update label (highlighting is done by
        the cursor selection from find())."""
        if not self._target:
            return
        # Count occurrences
        content = self._target.toPlainText()
        count = content.lower().count(text.lower())
        self.count_label.setText(
            f"{count} match{'es' if count != 1 else ''}")

    def _clear_highlight(self):
        """Remove any selection in the target."""
        if self._target:
            cursor = self._target.textCursor()
