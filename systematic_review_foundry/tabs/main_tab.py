...
class SectionEditor(QWidget):
    ...
    def __init__(self, section_key: str, display_name: str, parent_tab=None):
        super().__init__()
...
class MainReviewTab(QWidget):
    ...
    def _build_context_string(self, section_key: str) -> str:
        ...
        for s in self.session.sources:
            ...
            num = src.get('number')
            if not summary_nums or num in summary_nums:
    ...
    def swap_citation(self, section_key, citation_match, numbers):
        ...
        for num in numbers:
            ...
            reply = QMessageBox.question(
                self, f"Swap Citation [{num}]?", msg,
                QMessageBox.StandardButton.Yes |
                QMessageBox.StandardButton.No)

            if reply == QMessageBox.StandardButton.Yes:
                # Perform the swap in the editor text
                editor = self.section_editors[section_key]
                text = editor.get_text()
                # Replace only this specific citation occurrence
                # Use the match position to do a targeted replacement
                old_citation = citation_match.group(0)
                if len(numbers) == 1:
                    # Simple case: [5] -> [12]
                    new_citation = f'[{best_num}]'
                else:
                    # Multi-citation: [5,8] — just swap the one number
                    new_nums = [best_num if n == num else n
                                for n in numbers]
                    new_citation = '[' + ','.join(
                        str(n) for n in new_nums) + ']'
                # Replace at the known block position
                block_text = editor.editor.textCursor().block().text()
                # Find and replace in the full text
                # We need to be careful to replace only this occurrence
                new_text = (text[:text.find(old_citation)] +
                            new_citation +
                            text[text.find(old_citation) + len(old_citation):])
    ...
    def _validate_all_citations(self):
        reply = QMessageBox.question(
            self, "Validate All Citations",
            "This will use AI to validate all citations automatically. "
            "Continue?",
            QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No)
        if reply != QMessageBox.StandardButton.Yes:
            return
        ...
        self.section_changed.emit()
        QMessageBox.information(
            self, "Done", "Batch citation validation complete.")
