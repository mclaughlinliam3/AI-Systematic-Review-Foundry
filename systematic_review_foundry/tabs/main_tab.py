...
class CitationHighlighter(QSyntaxHighlighter):
    """
    Highlights [x] style citations, colouring each number by its own
    validation status.

    In a multi-citation like [3,7,12] the three numbers are independent
    elements, so they are formatted separately — 3 can be green while 7
    is red.  The surrounding brackets and commas are drawn in a neutral
    grey so the numbers read as distinct items.
    """
    ...
    def highlightBlock(self, text):
        for match in CITATION_RE.finditer(text):
            # Neutral base for the brackets/commas
            base = QTextCharFormat()
            base.setForeground(QColor(self.PUNCTUATION_COLOR))
            base.setFontWeight(QFont.Weight.Bold)
            self.setFormat(match.start(), match.end() - match.start(), base)

            inner_offset = match.start(1)
            for nm in re.finditer(r'\d+', match.group(1)):
                status = self.validation_data.get(
                    str(int(nm.group(0))), {}).get('status', 'unvalidated')
                fmt = QTextCharFormat()
                fmt.setForeground(QColor(self.STATUS_COLORS.get(
                    status, self.STATUS_COLORS['unvalidated'])))
                fmt.setFontWeight(QFont.Weight.Bold)
                fmt.setFontUnderline(True)
                self.setFormat(inner_offset + nm.start(),
                               nm.end() - nm.start(), fmt)
...
class SectionEditor(QWidget):
    ...
    def add_header_button(self, button):
        """Append an extra button to this section's header row."""
    ...
    def _context_menu(self, pos):
        ...
        # Work in absolute document coordinates rather than block-relative
        # ones: the downstream actions need to edit an exact location, and
        # a block offset alone can't identify which of several identical
        # citations was clicked.
        from citation_manager import span_at_position, part_at_position

        full_text = self.editor.toPlainText()
        abs_pos = self.editor.cursorForPosition(pos).position()
        span = span_at_position(full_text, abs_pos)

        if span:
            part = part_at_position(span, abs_pos)
            if part is not None:
                self._add_citation_actions(menu, span, part)

        menu.exec(self.editor.mapToGlobal(pos))

    def _add_citation_actions(self, menu, span, part):
        """
        Add citation actions for the single number the user clicked.

        For a multi-citation the clicked number is the target; the other
        numbers in the bracket are left completely alone.  Bulk actions
        for the whole bracket are still available in a submenu.
        """
        num = part['number']
        is_multi = len(span['numbers']) > 1
        suffix = f" in {span['match']}" if is_multi else ""

        menu.addSeparator()
        header = menu.addAction(f"── Citation [{num}]{suffix} ──")
        header.setEnabled(False)

        view_act = QAction(f"View Source [{num}]", self)
        view_act.triggered.connect(
            lambda: self.parent_tab.view_citation_sources([num]))
        menu.addAction(view_act)

        app_act = QAction(f"✓ Approve [{num}]", self)
        app_act.triggered.connect(
            lambda: self.parent_tab.validate_citation([num], "approved"))
        menu.addAction(app_act)

        dis_act = QAction(f"✗ Disapprove [{num}]", self)
        dis_act.triggered.connect(
            lambda: self.parent_tab.validate_citation([num], "disapproved"))
        menu.addAction(dis_act)

        auto_act = QAction(f"🔍 Auto-Detect [{num}] (regex)", self)
        auto_act.triggered.connect(
            lambda: self.parent_tab.auto_detect_citation(
                self.section_key, span, [num]))
        menu.addAction(auto_act)

        ai_act = QAction(f"✦ AI-Detect [{num}]", self)
        ai_act.triggered.connect(
            lambda: self.parent_tab.ai_detect_citation(
                self.section_key, span, [num]))
        menu.addAction(ai_act)

        swap_act = QAction(f"🔄 Find Best Source & Swap [{num}]", self)
        swap_act.triggered.connect(
            lambda: self.parent_tab.swap_citation(
                self.section_key, span, part))
        menu.addAction(swap_act)

        if not is_multi:
            return

        # ── Bulk actions on the whole bracket ──
        all_nums = list(span['numbers'])
        sub = menu.addMenu(f"Entire citation {span['match']}")

        view_all = QAction(f"View All Sources {span['match']}", self)
        view_all.triggered.connect(
            lambda: self.parent_tab.view_citation_sources(all_nums))
        sub.addAction(view_all)

        app_all = QAction("✓ Approve All", self)
        app_all.triggered.connect(
            lambda: self.parent_tab.validate_citation(all_nums, "approved"))
        sub.addAction(app_all)

        dis_all = QAction("✗ Disapprove All", self)
        dis_all.triggered.connect(
            lambda: self.parent_tab.validate_citation(all_nums, "disapproved"))
        sub.addAction(dis_all)

        auto_all = QAction("🔍 Auto-Detect All (regex)", self)
        auto_all.triggered.connect(
            lambda: self.parent_tab.auto_detect_citation(
                self.section_key, span, all_nums))
        sub.addAction(auto_all)

        ai_all = QAction("✦ AI-Detect All", self)
        ai_all.triggered.connect(
            lambda: self.parent_tab.ai_detect_citation(
                self.section_key, span, all_nums))
        sub.addAction(ai_all)
...
class MainReviewTab(QWidget):
    ...
    sources_renumbered = pyqtSignal()  # source numbering/order changed
    ...
    def _build_ui(self):
        ...
        self.renumber_btn = QPushButton("⇅ Renumber Sequentially")
        self.renumber_btn.setToolTip(
            "Renumber sources so those cited in the review become 1..n in "
            "order of first appearance, with uncited sources following "
            "after. Updates in-text citations, the reference list and the "
            "Sources tab.")
        self.renumber_btn.clicked.connect(self.renumber_sources_sequentially)
        citations_ed.add_header_button(self.renumber_btn)
    ...
    def _rebuild_citations(self):
        ...
        from citation_manager import (
            get_cited_numbers_in_order, build_reference_list)

        cited_numbers = sorted(set(get_cited_numbers_in_order(self.session)))
        if not cited_numbers:
            self._set_citations_text("")
            return

        self._set_citations_text(
            build_reference_list(self.session, cited_numbers))

    # ── Sequential renumbering ───────────────────────────────────────

    def _reload_section_texts(self):
        """Push session text back into every editor without re-triggering
        change signals (used after an out-of-band rewrite)."""
        for key in ("abstract", "intro", "methods",
                    "discussion", "conclusion"):
            if key in self.section_editors:
                self.section_editors[key].set_text(
                    getattr(self.session, key, None) or "")
        for i, rs in enumerate(self.session.results):
            key = f"results_{i}"
            if key in self.section_editors:
                self.section_editors[key].set_text(rs.get('text') or "")

    def renumber_sources_sequentially(self):
        """
        Renumber sources so cited ones are 1..n in order of first
        appearance and uncited ones follow at n+1..N, updating the in-text
        citations, the reference list and the Sources tab ordering.

        With no citations present this simply collapses the existing
        numbering to a gap-free sequence.
        """
        from citation_manager import (
            build_sequential_renumber_mapping, apply_renumbering)

        if not self.session.sources:
            QMessageBox.information(
                self, "No Sources",
                "There are no sources to renumber.")
            return

        plan = build_sequential_renumber_mapping(self.session)
        mapping = plan['mapping']
        if not mapping:
            QMessageBox.information(
                self, "Nothing to Renumber",
                "No numbered sources or citations were found.")
            return

        cited_count = len(plan['order'])
        unused_count = len(plan['unused'])

        if plan['is_noop']:
            QMessageBox.information(
                self, "Already Sequential",
                f"Numbering is already sequential.\n\n"
                f"Cited sources: {cited_count}\n"
                f"Uncited sources: {unused_count}")
            return

        # ── Confirmation, with the phantom warning up front ──
        detail = [
            f"  • {cited_count} cited source(s) → 1–{cited_count}"
            if cited_count else "  • No citations found in the review body",
        ]
        if unused_count:
            start = cited_count + 1
            detail.append(
                f"  • {unused_count} uncited source(s) → "
                f"{start}–{start + unused_count - 1}")
        body = (
            "Sources will be renumbered and reordered:\n\n"
            + "\n".join(detail) +
            "\n\nIn-text citations, the reference list, validation "
            "states, per-section context selections and the Sources tab "
            "order will all be updated to match.")
        if plan['phantom']:
            phantom_str = ', '.join(f'[{n}]' for n in plan['phantom'])
            body += (
                f"\n\n⚠ Warning: {phantom_str} "
                f"{'is' if len(plan['phantom']) == 1 else 'are'} cited in "
                f"the text but {'has' if len(plan['phantom']) == 1 else 'have'} "
                f"no matching source. "
                f"{'It' if len(plan['phantom']) == 1 else 'They'} will still "
                f"reserve a number so the text stays consistent, leaving a "
                f"gap in the reference list.")

        reply = QMessageBox.question(
            self, "Renumber Sequentially",
            body + "\n\nContinue?",
            QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No)
        if reply != QMessageBox.StandardButton.Yes:
            return

        summary = apply_renumbering(self.session, mapping)

        # Refresh every view that depends on source numbers
        self._reload_section_texts()
        for ed in self.section_editors.values():
            ed.update_validation_data(self.session.citation_validations)
        self._rebuild_citations()

        self.sources_renumbered.emit()
        self.section_changed.emit()

        msg = (f"Renumbering complete.\n\n"
               f"Cited sources: {cited_count} (now 1–{cited_count})\n"
               f"Uncited sources: {unused_count}\n"
               f"Total sources: {summary['sources_renumbered']}")
        if summary['dropped_validations']:
            msg += (f"\n\nDiscarded {summary['dropped_validations']} stale "
                    f"validation record(s) that referenced sources which "
                    f"no longer exist.")
        QMessageBox.information(self, "Renumbered", msg)
    ...
    def _get_citation_preceding_text(self, section_key, span):
        """
        ...
        `span` carries an absolute offset into the section text, so this
        no longer has to guess which block the citation lives in — the
        old block scan silently returned the wrong offset whenever the
        citation wasn't in the first block, or when the same citation
        string appeared more than once.
        """
    ...
    def auto_detect_citation(self, section_key, span, numbers):
        from citation_manager import auto_detect_match
        preceding = self._get_citation_preceding_text(section_key, span)
    ...
    def ai_detect_citation(self, section_key, span, numbers):
        preceding = self._get_citation_preceding_text(section_key, span)
        if preceding is None:
            return
    ...
    def swap_citation(self, section_key, span, part):
        """
        Find the best-matching source for ONE number within a citation and
        offer to swap just that number.

        In a multi-citation only `part` is considered a candidate for
        replacement; the other numbers in the bracket are untouched, and
        they are excluded from the search so the swap can't produce a
        duplicate like [2,2].
        """
        from citation_manager import (
        ...
        # Exclude every number already in this bracket, not just the
        # clicked one, so we never suggest a source that's already there.
        best = find_best_source_match(
            preceding, self.session.sources,
            self.citation_val_box.currentIndex(),
            exclude_numbers=list(span['numbers']))
        ...
        self._show_swap_dialog()

    def _show_swap_dialog(self):
        """Show a non-modal dialog comparing the current source with the
        best alternative for a single clicked citation number."""
        ...
        part = state['part']
        num = state['num']
        current_excerpt = state['current_excerpt']
        current_score = state['current_score']
        best_num = state['best_num']
        best_excerpt = state['best_excerpt']
        best_score = state['best_score']

        in_multi = len(span['numbers']) > 1
        context_note = (f"\n(Only [{num}] within {span['match']} will "
                        f"change.)" if in_multi else "")
        ...
        if best_score > current_score:
            msg += (
                ...
                f"Swap [{num}] → [{best_num}]?{context_note}")
        else:
            msg += (
                f"Current source [{num}] is already the best or "
                f"equal match. Swap anyway to [{best_num}]?{context_note}")

        dlg = QDialog(self)
        title_suffix = f" in {span['match']}" if in_multi else ""
        dlg.setWindowTitle(f"Swap Citation [{num}]{title_suffix}?")
        dlg.setModal(False)
        layout = QVBoxLayout(dlg)
        ...
        def on_swap():
            from citation_manager import replace_citation_number
            editor = self.section_editors[section_key]
            text = editor.get_text()

            # The dialog is non-modal, so the text may have been edited
            # while it was open. Verify the recorded offsets still point
            # at the citation we scored before touching anything.
            if text[span['start']:span['end']] != span['match']:
                dlg.close()
                QMessageBox.warning(
                    self, "Text Changed",
                    f"The text around citation {span['match']} has changed "
                    f"since this comparison was generated, so the swap was "
                    f"not applied. Please right-click the citation again.")
                return

            new_text, new_bracket = replace_citation_number(
                text, span, part, best_num)

            # Preserve the viewport so the editor doesn't jump to the top
            scrollbar = editor.editor.verticalScrollBar()
            ...
            if in_multi:
                detail = (f"{span['match']} → {new_bracket}"
                          if new_bracket != span['match'] else new_bracket)
                QMessageBox.information(
                    self, "Swapped",
