...
from dialogs import PromptApprovalDialog, OutputApprovalDialog, ContextConfigDialog
...
def _top_n_sources_for_topic(session, topic_id: str, n: int = 10):
    ...
    for a given topic_id.  Falls back to all summaries if no ratings exist.
    """
    rated, unrated = [], []
...
def _top_n_source_list(session, topic_id: str, n: int = 10):
    ...
    preserving order for iteration.
    """
    rated, unrated = [], []
    for s in session.sources:
        src = s if isinstance(s, dict) else s
...
def _top_n_sources_for_stat(session, stat_id: str, n: int = 10):
    """
    Return a dict {source_number: summary} of the top-n rated sources
    for a given stat_id. Falls back to all summaries if no ratings exist.
    """
    rated, unrated = [], []
    for s in session.sources:
        src = s if isinstance(s, dict) else s
...
class TopicsTab(QWidget):
    ...
    def _build_ui(self):
        ...
        auto_btn = QPushButton("✦ Auto-Generate Topics")
        ...
        gen_q_btn = QPushButton("✦ AI-Generate Questions")
    ...
    def _auto_generate(self):
        ...
        for s in self.session.sources:
            src = s if isinstance(s, dict) else s
            if src.get('summary'):
                source_dict[src.get('number', 0)] = src['summary'][:200]
    ...
    def _ai_write_topic_bulk(self, row):
        """Original bulk mode: all sources in one prompt."""
        topic = self.session.topics[row]
        topic_id = topic.get('topic_id', '')
        n = self.topic_top_n.value()
        source_dict = _top_n_sources_for_topic(
            self.session, topic_id, n)

        variables = {
            'topic': self.session.paper_topic,
            'topic_title': topic.get('title', ''),
            'context': str(source_dict)[:8000],
        }
    ...
    def _ai_write_topic_iterative(self, row):
        ...
        topic_id = topic.get('topic_id', '')
        n = self.topic_top_n.value()
        sources = _top_n_source_list(self.session, topic_id, n)
    ...
    def _link_to_sections(self):
        ...
        if row < 0:
            return
        sections = ['intro', 'discussion', 'conclusion']
        for rs in self.session.results:
            sections.append(f"results: {rs.get('section', '?')}")
        text, ok = QInputDialog.getText(
            self, "Link to Sections",
            f"Enter section names to link (comma-separated):\n"
            f"Available: {', '.join(sections)}")
        if ok:
            linked = [s.strip() for s in text.split(',') if s.strip()]
...
class StatsTab(QWidget):
    ...
    def _get_context_string(self, row):
        ...
        source_dict = _top_n_sources_for_stat(
            self.session, stat_id, n)
        return str(source_dict)[:8000]
    ...
    def _query_text_iterative(self, row):
        ...
        n = self.stat_top_n.value()
        stat_id = stat.get('stat_id', '')
        sources = _stat_source_list(self.session, stat_id, n)
    ...
    def _link_to_sections(self):
        ...
        text, ok = QInputDialog.getText(
            self, "Link to Sections",
            "Enter section names to link (comma-separated):")
        if ok:
            linked = [s.strip() for s in text.split(',') if s.strip()]
