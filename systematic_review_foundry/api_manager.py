...
from config_manager import build_claude_payload, extract_text
...
class APIManager:
    ...
    def _query_claude(self, prompt: str, max_tokens: int, temperature: float,
        ...
        if not api_key:
            raise ValueError(
                "No Claude API key configured. Please set it in Prompt Settings.")
     
        # Caller may override, but default to the configured model so this
        # stays consistent with every other call path.
        model = model or self.config.claude_model
        if not model:
            raise ValueError(
                "No Claude model selected. Open Prompt Settings and choose one.")
        ...
        # Decides which of temperature / top_p (if any) this model accepts,
        # and clamps max_tokens to its ceiling.
        data = build_claude_payload(model, prompt, max_tokens,
        ...
        while retries < max_retries:
            try:
                ...
                # NOT result['content'][0]['text'] -- on thinking models the
                # first block is a thinking block, so that raises
                # KeyError('text'), which surfaces as the bare message 'text'.
                ...
                if status in (429, 529):
                    ...
                    hint = ""
                    if status == 400 and not is_known_model(model):
                        hint = (f"\n\nNote: '{model}' is not in this build's "
                                f"model table, so only generic parameters were "
                                f"sent. It may require parameters this version "
                                f"does not know about.")
                    raise RuntimeError(
                        f"Claude API error: {status} — {e.response.text}{hint}")
