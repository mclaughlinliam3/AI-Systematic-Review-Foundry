...
def _construct_citation(article, doi) -> Tuple[str, str, str]:
    ...
    """
    def _end_period(s):
        """Terminate with a period unless already terminated. Empty -> empty."""
        s = str(s or "").strip()
        if not s:
            return ""
    ...
    for author in authors:
        ...
        if last is not None and init is not None:
            author_list.append(f"{_text(last)} {_text(init)}".strip())

    title_el = article.find(".//ArticleTitle")
    title = (_get_element_text(title_el).strip()
             if title_el is not None else "Unknown Title")
    journal = _text(article.find(".//Journal/Title"), "Unknown Journal")

    year = _text(article.find(".//PubDate/Year"))
    if not year:
        year = _text(article.find(".//PubDate/MedlineDate"))[:4] or "Unknown Year"

    volume = _text(article.find(".//Volume"))
    issue = _text(article.find(".//Issue"))
    pages = _text(article.find(".//MedlinePgn"))
    ...
    segments = [authors_str, title, journal, locator]
    if doi:
        segments.append(f"doi: {doi}")

    citation = " ".join(_end_period(s) for s in segments if _end_period(s))
